...
try:
    ...
    print(
        "⚠️  crewai / langchain_community not installed.\n"
        "   pip install crewai langchain-community duckduckgo-search python-dotenv"
    )
...
MODEL_RIGID = os.environ.get("FINSIGHT_LLM_RIGID", "gemini/gemini-2.5-flash")
MODEL_CREATIVE = os.environ.get("FINSIGHT_LLM_CREATIVE", "gemini/gemini-2.5-flash")
...
def run_factory(job: ArticleJob, interactive_alpha: bool = True) -> FactoryResult:
    if not CREWAI_AVAILABLE:
        raise RuntimeError("Install: pip install crewai langchain-community duckduckgo-search python-dotenv")
