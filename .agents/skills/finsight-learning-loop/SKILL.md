...
```json
{
  ...
  "rules_under_test": ["S2", "S5"],
  "input_is_excerpt": true,
  "rule_id_provenance": "note cites S7/S8; S7 -> S2 after the Floor/Mandate refactor"
}
```

**`rules_under_test` is mandatory, and it is not bookkeeping.** Every case is a
short excerpt written to probe one or two rules. Without scoping, the evaluator
applies the whole rubric and fails a prose fragment on M1 (no publishable MDX
structure) and M5 (no `[SVG_PROPOSAL]` tags) — criteria an excerpt cannot
satisfy and was never written to demonstrate. That is exactly what happened:
the gate degraded silently as R7 and the SVG requirement were added, and by
15 Aug both PASS cases were failing for irrelevant reasons.

The quieter half is worse. Both FAIL cases kept "passing", because a fragment
fails *something* — so `should_fail` succeeded whether or not the rule under
test ever fired. Those cases exist to catch a rubric quietly loosening, and on
structural grounds they could never have done it. **An assertion that still
holds when the thing under test is removed is not an assertion.** See sol-020.

**Write `rules_under_test` in the current namespace**, and record the historical
ids in `rule_id_provenance`. The original case notes cited S7, S8 and M10 — the
pre-refactor namespace — so the regression set had been orphaned by the same
renumbering that orphaned the failure log. Resolve old ids through
`rule_registry.py` rather than by memory.
