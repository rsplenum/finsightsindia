...
# Text default: 2.5-flash is blocked for many new API keys ("no longer available to new users")
_DEFAULT_TEXT = "gemini/gemini-3.5-flash"
# Nano Banana 2 — image model (white-blend Varsity, dark-mode safe)
...
# Auto-replace retired / blocked text models (do not touch *image* model ids)
_RETIRED_TEXT = (
    "gemini-2.5-flash",
    "gemini-2.0-flash",
    "gemini-1.5-flash",
...
if "image" not in MODEL_TEXT.lower():
    if any(r in MODEL_TEXT for r in _RETIRED_TEXT):
        print(f"⚠️  Text model {MODEL_TEXT!r} is retired/blocked → {_DEFAULT_TEXT!r}")
        MODEL_TEXT = _DEFAULT_TEXT
    elif "gemini-3.1-pro" in MODEL_TEXT and "preview" not in MODEL_TEXT:
        print(f"⚠️  Text model {MODEL_TEXT!r} invalid → {_DEFAULT_TEXT!r}")
        MODEL_TEXT = _DEFAULT_TEXT
...
- Focus on the DELTA — the marginal difference that produces an entirely different result.
- Focus on MECHANISM — step-by-step from initial state to final state.
- Find the small legal or procedural hinge that changes the entire outcome.

THE 5 WHYS (historical intent — mandatory):
What existed before? Why did the government create this rule? Why are people trapped now?
Lead the reader to the intent behind the code, not only the text of the code.
"""

# FinSight Content & Strategy Blueprint — floor for every article agent
CONTENT_STRATEGY = """
═══════════════════════════════════════════════════════════════
FINSIGHT CONTENT STRATEGY (FLOOR, NOT CEILING)
═══════════════════════════════════════════════════════════════

AUDIENCE
- High-intent, high-stakes citizens (engineers, freelancers, founders, inheritors).
- Common people under real money pressure — NOT CAs, NOT auditors.
- They will read 20–30 minutes if every paragraph earns its place.

LANGUAGE (non-negotiable)
- Simple, modern, impactful. Dejargonise. Demystify.
- Prefer "the tax computer matched your AIS" over "CPC algorithmic telemetry".
- Prefer "fake bills" over "accommodation entries" until you define the term once.
- No cold audit-memo voice. No "Case File / Resolution Status / File closed".
- Dense with utility. Zero fluff. Never open with "Taxes are the backbone of a nation."

HARD BAN — ASCII / TEXT DIAGRAMS
- NEVER use ASCII art, box-drawing trees, markdown flowcharts, or
  ```
  [A] → [B] → [C]
  ```
  style pipelines.
- Whatever needs a diagram MUST be described as an illustration beat for the art pipeline
  (stick-figure / Varsity minimalist infographic). Write a short line like:
  [Illustration: stick-figure path from AIS mismatch to penalty notice]
  Do not draw it in characters.

REQUIRED STRUCTURE (minimum floor — order may flex with topic vibe)
1. Symptom-led opening (human stakes, search-intent language).
2. TL;DR within the first screen — use a CardPremium-style block with 3–5 bullets
   that answer the reader's question before the deep dive.
   Example shape:
   <CardPremium>
   ### TL;DR
   - ...
...
3. First-principles: why the law exists (history / intent).
4. Mechanism in plain language (how the system actually works).
5. Scenario matrix or comparison table when numbers differ by person
   (e.g. three income levels or two legal paths).
6. Notice traps: what the department / CPC looks for.
7. Actionable checklist (human verbs: Download…, File…, Do not…).
8. KEY TAKEAWAYS at the end (5–8 bullets) — what to remember on Monday morning.
9. Closing return to the person: agency, not a lecture.

ILLUSTRATIONS
- Do not limit to two. If the topic needs 6–10 inset beats, list 6–10
  [Illustration: …] markers at the right narrative moments.
- Anxiety, mechanism, trap, relief — each may need its own image.

STANDARDS ARE A PUT OPTION
- TL;DR, Notice traps, checklist, takeaways = floor.
- If a rule blocks a better human story, break the rule — but never drop outcome certainty.
- Substantive justice over arbitrary word-count ceilings.
...
def build_agents(search_tool: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "novelist": Agent(
            ...
            goal=(
                "Write a demystifying article for common people: simple modern impactful language, "
                "TL;DR near the top, key takeaways at the end, outcome certainty. "
                "Honor gaps + alpha. No ASCII diagrams."
            ),
            backstory=(
                DEEP_ANALYSIS_MANDATE
                + "\n\n"
        ...
        "reader": Agent(
            ...
            goal=(
                "PASS only if a non-CA can decide and act. Enforce Content Strategy floor: "
                "TL;DR present, key takeaways present, no ASCII diagrams, language dejargonised, "
                "gaps covered, outcome certainty."
            ),
            backstory=(
                DEEP_ANALYSIS_MANDATE
                + "\n\n"
        ...
        "seo": Agent(
            role="SEO Strategist",
            goal="JSON: symptom headline, meta description, faqs — outcome certainty, not dictionary titles.",
            backstory="Search intent = panic symptom + decision.",
            llm=llm_creative,
            allow_delegation=False,
            verbose=True,
        ),
        "art": Agent(
            ...
            goal=(
                "From the article, list EVERY illustration beat needed (not capped at 2). "
                "Include anxiety, mechanism/infographic, trap, and relief beats when present. "
                "Varsity / stick-figure friendly scene prompts only."
            ),
            backstory=(
                "FinSight visual language: minimalist Varsity or clear stick-figure infographics on white. "
                "Replace any ASCII diagram the novelist left as [Illustration: …] with a real scene prompt. "
                "No generic person-at-desk. No background color instructions (pipeline forces white plate)."
            ),
            llm=llm_creative,
            allow_delegation=False,
            verbose=True,
...
def t_seo(job: ArticleJob, agent: Any, article: str) -> Any:
    return Task(
        description=(
            ...
            "title, description, readTime (honest estimate), statutoryAct, faqs:[{q,a}], intent\n"
            "Title = high-anxiety symptom + outcome, not dictionary definition / not 'Section X of Act'."
        ),
...
def t_art(agent: Any, article: str) -> Any:
    return Task(
        description=(
            f"{CONTENT_STRATEGY}\n\n"
            "Scan the article for every [Illustration: …] marker and every place that still needs a visual "
            "(anxiety open, mechanism, trap, comparison, relief).\n\n"
            "Return JSON:\n"
            '{"illustrations":[{"id":"anxiety","prompt":"≤30 words scene","alt":"..."}, ...]}\n\n'
            "Rules:\n"
            "- As many illustrations as the topic needs (typically 4–8; not stuck at 2).\n"
            "- Scene content only. Stick-figure or Varsity minimalist subjects.\n"
            "- No text inside the image. No background color (pipeline forces pure white plate).\n"
            "- If the draft still contains ASCII diagrams, convert each into an illustration prompt."
        ),
        expected_output="JSON list of illustration prompts.",
        agent=agent,
    )
...
def run_factory(job: ArticleJob | None = None) -> FactoryResult:
    ...
    try:
        ...
        # 6 Nano Banana images (N illustrations from art director)
        logger.section("Phase E — Image generation (Nano Banana)")
        image_rels: list[tuple[str, str]] = []
        image_paths: list[str] = []
        slug = job.filename
        illust_list = art.get("illustrations")
        if isinstance(illust_list, list) and illust_list:
            prompts = [
                (
                    (item.get("prompt") or f"Varsity scene for {job.topic}"),
                    (item.get("alt") or item.get("id") or f"Illustration {i}"),
                    (item.get("id") or f"illust-{i}"),
                )
                for i, item in enumerate(illust_list)
                if isinstance(item, dict)
            ]
        else:
            # Backward compatible if model returns old shape
            prompts = [
                (art.get("anxiety_prompt") or f"Anxiety moment for {job.topic}", art.get("anxiety_alt") or "Anxiety", "anxiety"),
                (art.get("relief_prompt") or f"Clarity and relief for {job.topic}", art.get("relief_alt") or "Relief", "relief"),
            ]
        # Cap runaway generation costs
        max_images = int(os.environ.get("FINSIGHT_MAX_IMAGES", "8"))
        prompts = prompts[:max_images]

        for i, (prompt, alt, iid) in enumerate(prompts):
            safe_id = re.sub(r"[^a-z0-9\-]+", "-", str(iid).lower()).strip("-") or f"illust-{i}"
            fname = f"{slug}-{safe_id}.jpg"
            abs_path = IMAGES_ROOT / fname
            logger.note(f"Image {i} ({safe_id}): `{prompt}` → `{abs_path}`")
        ...
        # Strip residual ASCII-ish box drawing if model slipped
