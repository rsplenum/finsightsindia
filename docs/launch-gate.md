...
- [ ] **WAITING ON RAHUL — the ~~21~~ 14 nine-pixel labels.** *(Has been recorded as 17, 20 and 21 at various moments — see sol-058. **14** is the count at HEAD after this session, and the test now holds that.)* The fall is not a sweep: the tax revamp took the regime chips and the insurance overhaul took its three. What is left, named: `CAUTIOUS`/`BALANCED`/`BOLD` on SIP, the five Greeks on the options page, one on the planner, two on `TaxAnswer`, one illustration. Going 9px→12px reflows the badges they sit in, so it is a layout decision rather than a find-and-replace. The test caps the count so the deferral cannot quietly grow. **Also held: the 11px `INDIA` in the header wordmark** — a brand lockup, not body text; changing it alters the logo. Remaining after those: `faq.astro` (14) and the content workstream's fcnr illustrations (~30)
