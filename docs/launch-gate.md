...
- [ ] **THE CRITICAL PATH, per D-1: rebrand the four trust pages, then ship.** They still say "SWP Intelligence Engine" with a `swpplanner.com` support address; 0 occurrences of "FinSight India". Needs a free sol- id (see the line below). D-5's author bio lands in the same pass, and the bio copy is shown to Rahul before it goes live
- [ ] **Tax calculator revamp — Rahul's sketch, 16 Aug.** One adaptive form: Category (Salaried / Business / Professional / Services) · Name · Age, then **Income** built additively (Salary always present, a dropdown ADDS any other source) and **Deductions** the same way (standard deduction always present, a dropdown adds the rest), then one Tax computation panel. Dropdown 1 = income heads, dropdown 2 = capital gains, F&O, freelancing, gig economy. **Reading to confirm with Rahul before building**: that the Category selection *filters* what both dropdowns offer, and that a dropdown-that-adds-a-row replaces T6's four `<details>` chips rather than sitting beside them. One real trade-off to raise: a chip advertises what exists, a dropdown hides it behind a click — a reader who does not know F&O is a category will not go looking for it (dd-009)
...
- [x] ~~**D-1 · Ship now, or close the blockers first?**~~ **Answered 16 Aug: ship in week 1, but only after the trust pages are rebranded.** Agreed in full. Three weeks of waiting costs three weeks of indexing and buys very little; the trust pages are the one defect that actively damages us while live. **This makes the rebrand the critical path** — and its plan id `sol-048` is already taken, see above
