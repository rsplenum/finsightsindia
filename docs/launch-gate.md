...
- [ ] **THE CRITICAL PATH is now the chassis, not the launch date (D-1 reversed).** In order: the Phase 2 token sweep (contrast, type floor, emphasis budget — the three rules D-2 sent to lint), the insurance analyser overhaul, the tax calculator revamp from Rahul's sketch. Launch follows all three. Rahul, 16 Aug: *"the low hanging fruit first then the tough job"*
- [ ] **F-02 measured in the browser, not asserted — swp-planner, both themes.** Light **23 groups / 45 nodes** fail AA; dark **7 groups**. The audit's headline reproduces exactly (`text-gold-600` = **2.88:1**), but **`gold-500` is worse at 1.88–2.10:1 and carries the single biggest number on the page** (`₹ 2.91 Cr`, 36px, needs 3:1). Also failing: `navy-400` 2.56 · `slate-400` 2.63 (a stray Tailwind stock palette beside our navy) · `amber-600` 3.06 · `emerald-600` 3.49 · `navy-500` 3.75–4.44 (marginal) · two badge classes with no `text-` class at 2.95 and 3.43. **Method matters and cost three wrong answers**: counting `text-*` classes statically is wrong (ignores the real ground), regex-parsing colours is wrong (Tailwind v4 emits `oklch`, so `0.869` was read as a red channel), and screenshotting before `.reveal`'s 0.7s fade completes is wrong (it reported a washed-out hero that is actually fine). Ground truth = canvas-resolved colour, alpha composited up the ancestor chain, gradients excluded, mid-animation nodes excluded. **Gold half done, 16 Aug**: Rahul chose **option F** — the headline figure is navy with the brand gold as the rule beneath it, and `gold-700 #836911` is now the one gold allowed on light text (`gold-500`/`gold-600` keep the brand and stay for rules, borders and all dark-theme text). 76 usages codemodded across 24 files plus `Layout.astro`. **23 groups/45 nodes → 16/36.** One gold instance survives: the `FinSight INDIA` wordmark at 3.40:1, which sits on a mid-tone ground where neither the dark gold nor the bright one clears AA — it needs a look at the header's own background, not another token. Still open: `navy-400` as light text (13 nodes) · `slate-400` (10) · `amber-600` · `emerald-600` · two badge classes carrying no `text-` class · `navy-500` marginal at 3.75–4.44
