...
- [ ] **D-1 · Ship now, or close the blockers first?** Indexing age only accrues once live, and AdSense needs ~2 months of history. Recommendation: ship in week 1, but only after the trust pages are rebranded — that is the one defect that gets worse the longer the site is up
- [ ] **D-2 · Do four visual rules become doctrine?** Proposed from the audit, in an agent's words not yours: type has a floor · contrast is a property of the pair and light is the theme strangers see · emphasis is a budget · a number is a quantity, an amount and a frame, never split apart. **They cannot be written as dd- entries without your ruling** — all 19 existing entries carry your verbatim words, and inventing a quote to fill that field would corrupt the one store that depends on being real. Recommendation: take the fourth at minimum; it caused two separate findings
- [ ] **D-3 · Which rupee convention wins?** Ten `formatRupee()` definitions, two live conventions, both on the SWP planner: `₹1.28 crore` (answer layers) vs `₹ 1.50 Cr` (shared utility). Blocks sol-055. Recommendation: the word, no space, one decimal above a lakh
- [ ] **D-4 · Publish 54 articles, or hold the thin ones?** 38 sit between 1,000–1,500 words; 13 already fail R2, 26 fail R7. AdSense assesses the site, not the best of it. Recommendation: hold everything failing R2 — they lose nothing, being unindexed
- [ ] **D-5 · Who is the author?** No human anywhere on the site; JSON-LD says `Organization`. YMYL content needs a named person with credentials. Also settles whether the AI-assisted pipeline is disclosed. Recommendation: disclose it plainly — with a solutions ledger already published, that reads as integrity
- [ ] **D-6 · Five calculators broad, or the insurance analyser deep?** The audit argues for narrowing: it has the sharpest question, the most surprising answer and the least competition, and being unfunded is an advantage there rather than a handicap. Recommendation: narrow — but not before the tax page is finished, since half-migrated is worse than either end
